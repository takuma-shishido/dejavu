...
def home(request):
    ...
    # model = Novels
    return render(request, "home/home.html", {'novels': novels, 'top_novel': top_novel, 'middle_novels': middle_novels})
...
class CreateNovelView(generic.CreateView):
    ...
    def setInitial(self, request):
        ...
        form = NovelCreateForm(initial={'user_id': request.user.user_id})
        return self.object

def createNovel(request):
    novels = Novels.objects.all()
    form = NovelCreateForm(request.POST, initial=request.user.user_id)
    context = {
        'novels': novels,
        'form': form,
    }
